...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach((e)=>{
   let findProduct =  PRODUCTS.find(element => element.id == e.id)
   let subtotal = e.quantity * findProduct.price
   result+= subtotal
  })
...
function addProductToCart(productId) {
  // Write your code here
  const product = SHOPPING_CART.find(product => productId == product.id)
...
function removeProductFromCart(productId) {
  // Write your code here
  let indexx = 0;
  let product = SHOPPING_CART.find((item , index) => {
